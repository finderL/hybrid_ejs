 ...
 * 通过JSBridge交互,异步
 * @author dailc
 * @version 2.0  2.0版本的备份，因为2.1绝对统一风格，不再支持2.0的参数传递方式
 * @time 2016-12-02
 */
(function(exports, isLocal) {
	...
	ejs.nativeUI = {
		 ...
		 * @param {String} title 标题
		 * @param {String} msg 消息内容
		 * @param {Array} btnArray 按钮数组,只支持两位
		 * @param {Function} callback 回调函数
		 * @param {JSON} options 额外配置参数
		 */
		confirm: function(title, msg, btnArray, callback, options) {
		 ...
		 * @param {String} defaultTxt 默认文本
		 * @param {Function} callback 回调函数
		 * @param {JSON} options 额外配置参数
		 */
		prompt: function(title, hint, defaultTxt, callback, options) {
			options = options || {};
			//目前按钮名称不能自定义
		 ...
		 * 异步
		 * @param {String} title 标题
		 * @param {Array} itemArray item数组
		 * @param {Array} choosArray 设置为选择的array,multi时才有效
		 * @param {Function} callback 回调函数
		 * @param {JSON} options 额外配置参数
		 */
		select: function(title, itemArray, choosArray, callback, options) {
			itemArray = itemArray || [];
			choosArray = choosArray || [];
			options = options || {};
			//目前按钮名称不能自定义
			JSBridge.callHandler(EJS_API, 'showSelectedDialog', {
				'title': title,
				'items': itemArray.join(','),
				'checkState': choosArray.join(','),
				//默认为可取消
				'cancelable': options.cancelable || 0,
		 ...
		 * 异步
		 * @param {String} title 标题
		 * @param {String} datetime 指定日期，默认当前日期。格式为 yyyy-MM-dd。
		 * @param {Function} callback 回调函数
		 */
		pickDate: function(title, datetime, callback) {
			//目前按钮名称不能自定义
		 ...
		 * 异步
		 * @param {String} title 标题
		 * @param {String} datetime 指定日期，默认当前日期。格式为 yyyy-MM-dd HH:mm
		 * @param {Function} callback 回调函数
		 */
		pickTime: function(title, datetime, callback) {
			//目前按钮名称不能自定义
		 ...
		 * cancelable默认为true，且无法设置
		 * 异步
		 * @param {String} title1 日期选择控件标题
		 * @param {String} title2 时间选择控件标题
		 * @param {String} datetime 指定日期，默认当前日期。格式为 yyyy-MM-dd。
		 * @param {Function} callback 回调函数
		 */
		pickDateTime: function(title1, title2, datetime, callback) {
			//目前按钮名称不能自定义
		 ...
		 * iOS 不支持cancelable参数设置
		 * 异步
		 * @param {Array} itemArray 内容
		 * @param {Function} callback 回调函数
		 * @param {JSON} options 额外配置参数
		 */
		actionSheet: function(itemArray, callback, options) {
			itemArray = itemArray || [];
			options = options || {};
			//目前按钮名称不能自定义
		 ...
		 * @param {JSON} options 额外配置参数
		 * orientation
		 * 菜单类别。仅为"horizontal"时弹出横向菜单，否则弹出垂直菜单。
		 */
		popWindow: function(itemArray, callback, options) {
			options = options || {};
			itemArray = itemArray || [];
			var titleArray = [];
			var iconArray = [];
			for(var i = 0, len = itemArray.length; i < len; i++) {
				titleArray.push(itemArray[i].title);
				iconArray.push(itemArray[i].icon);
			}
			JSBridge.callHandler(EJS_API, 'showPopupWindow', {
				'titleItems': titleArray.join(','),
				'iconItems': iconArray.join(','),
	...
	ejs.navigator = {
		 ...
		 * @param {Function} callback 回调函数,这个回调函数仅仅通知是否调用成功
		 */
		setRightImageBtn: function(localImgUrl, callback) {
		 ...
		 * @param {Function} callback 回调函数
		 */
		showSearchBar: function(callback) {
		 ...
		 * @param {Function} callback 回调函数
		 */
		hideSearchBar: function(callback) {
	...
	ejs.device = {
		 ...
		 * @param {Function} callback 回调函数
		 */
		callPhone: function(phone, callback) {
		 ...
		 * @param {Function} callback 回调函数
		 */
		sendMsg: function(phone, message, callback) {
	...
	ejs.nativeComponents = {
		 ...
		 * @param {JSON} options 额外参数
		 * @param {Array} urlArray 图片地址。网络图片以"http://"开头；设备本地路径以"file://"开头，
		 * 后面接文件sd卡路径，；用drawable中的资源文件以"drawable://"开头，
		 * 后面接图片名字；应用assets中的资源文件以"assets://"开头，后面接文件路径。
		 * @param {Number} index 默认显示图片序号。值为数字
		 */
